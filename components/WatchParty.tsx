      ...
      {!joined ? (
        <div className="flex flex-wrap gap-2">
          <input
            ...
            className="min-w-0 flex-1 rounded-lg border border-zinc-700 bg-zinc-800 px-3 py-2 font-mono text-xs uppercase text-white placeholder:text-zinc-500 focus:border-rose-600 focus:outline-none"
          />
          <button
            onClick={() => roomId && connect(roomId)}
            disabled={!roomId}
            className="rounded-lg bg-rose-600 px-3 py-2 text-xs font-semibold text-white transition-colors hover:bg-rose-500 disabled:opacity-40"
          >
            Join
          </button>
          <button
